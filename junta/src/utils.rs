...
pub enum OneOfFour<F1: Future, F2: Future, F3: Future, F4: Future> {
    Future1(F1),
    Future2(F2),
    Future3(F3),
    Future4(F4),
...
pub struct OneOfFourFuture<F1: Future, F2: Future, F3: Future, F4: Future> {
    inner: OneOfFour<F1, F2, F3, F4>,
}

impl<F1: Future, F2: Future, F3: Future, F4: Future> OneOfFourFuture<F1, F2, F3, F4> {
    pub fn new(inner: OneOfFour<F1, F2, F3, F4>) -> OneOfFourFuture<F1, F2, F3, F4> {
        OneOfFourFuture { inner }
    }
}

impl<F1: Future, F2: Future, F3: Future, F4: Future> Future for OneOfFourFuture<F1, F2, F3, F4>
where
    F1: Future<Item = (), Error = JuntaError>,
    F2: Future<Item = (), Error = JuntaError>,
    F3: Future<Item = (), Error = JuntaError>,
    F4: Future<Item = (), Error = JuntaError>,
{
    type Item = ();
